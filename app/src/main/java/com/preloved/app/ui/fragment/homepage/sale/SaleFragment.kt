...
class SaleFragment : BaseFragment<FragmentSaleBinding, SaleViewModel>
    ...
    private val accept = "accepted"
    private val sold = "sold"
    ...
    override fun setOnClickListeners() {
                ...
                viewModel.getHistoryResult().observe(viewLifecycleOwner) {
                    when (it) {
                        ...
                        is Resource.Success -> {
                            ...
                            showLoading(false)
                            val saleHistoryAdapter =
                                SaleHistoryAdapter(object : SaleHistoryAdapter.OnClickListener {
                                    override fun onClickItem(data: HistoryResponseItem) {
                                        val passData =
                                            SaleFragmentDirections.actionSaleFragmentToDetailProductFragment2(
                            ...
                            val sorted = it.data?.sortedByDescending { data -> data.id }
                                ?.filter { res -> res.status == accept && res.product.status == sold }
        ...
        getViewBinding().btnTerjual.setOnClickListener {
            ...
            viewModel.getSellerProductOrderResult().observe(viewLifecycleOwner) { res ->
                when (res) {
                    is Resource.Loading -> {
                        showLoading(true)
                    }
                    is Resource.Success -> {
                        ...
                        showLoading(false)
                        val saleOrderAcceptedAdapter =
                            SaleAcceptedAdapter(object : SaleAcceptedAdapter.OnClickListener {
                                override fun onClickItem(data: SellerOrderResponse) {
                                    bundlePenawar.putInt(ORDER_ID, data.id)
                                    val passData =
                                        SaleFragmentDirections.actionSaleFragmentToBuyerInfoFragment(
                                            productId = data.productId,
                                            orderId = data.id
                                        )
                        ...
                        val sortedSell = res.data?.sortedByDescending { it.id }
                        ...
                        if (res.data?.size == 0 || sortedSell?.size == 0) {
                            getViewBinding().apply {
                                lottieEmpty.visibility = View.VISIBLE
                                tvLottieEmpty.visibility = View.VISIBLE
                ...
                when (res) {
                    ...
                    is Resource.Success -> {
                        ...
                        if (res.data != null) {
                            ...
                            val sorted = res.data.sortedByDescending { it.id }.filter { it.product.status != "sold" }
                            ...
                            if (res.data.isEmpty() || sorted.isEmpty()) {
                                getViewBinding().apply {
                                    lottieEmpty.visibility = View.VISIBLE
                                    tvLottieEmpty.visibility = View.VISIBLE
    ...
    override fun observeData() {
        viewModel.userSessionResult().observe(viewLifecycleOwner) { it ->
            ...
            viewModel.getSellerProductResult().observe(viewLifecycleOwner) { res ->
                when (res) {
                    is Resource.Loading -> {
                        getViewBinding().apply {
                            //VISIBLE
